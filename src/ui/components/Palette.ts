...
export class Palette {
  ...
  private expandedGroupIndex: number | null = null; // Индекс открытой группы (меню)
  ...
  constructor(state: State) {
    ...
    // Закрываем меню при клике/тапе вне палитры (на холст)
  ...
  // Генерация круговой заливки для отображения группы цветов
  ...
  public render() {
    // ВАЖНО: Мы больше не перезаписываем this.container.className!
    // Классы из index.html (pointer-events-auto, flex-col и т.д.) сохраняются.
    ...
    colorsArr.forEach((item, i) => {
      ...
      if (typeof item === "string") {
        ...
        // Нажатие и удержание (редактирование цвета)
        ...
        btn.addEventListener("pointerup", (e) => {
          ...
          if (!this.isLongPress) {
            ...
            this.expandedGroupIndex = null; // Закрываем другие меню
            this.render();
          }
        });
        ...
        // Всплывающее меню с цветами (открывается ВПРАВО, так как палитра слева)
        if (isExpanded) {
          const subMenu = document.createElement("div");
          // left-14 смещает меню вправо от кнопки.
          subMenu.className =
          ...
          item.colors.forEach((color: string, j: number) => {
            ...
            cBtn.addEventListener("pointerup", (e) => {
              ...
              if (!this.isLongPress) {
                ...
                this.expandedGroupIndex = null; // Закрыть меню после выбора
                this.render();
              }
            });
